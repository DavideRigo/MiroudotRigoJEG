* =============================================================================
* This do file replicates Table B1
* =============================================================================
...
bys cou_par (year): gen rta_age = sum(rta)
...
ppml_panel_sg fa_outgood rta rta_age, ex(par) im(cou) y(year) 
outreg2 using "$home/output/results/tableB1", ///
replace text addtext(Host-year FE, YES, Partner-year FE, YES, Country-pair FE, YES) label cttop("GOOD")

ppml_panel_sg fa_outgood rta inv_prov rta_age, ex(par) im(cou) y(year) // 
outreg2 using "$home/output/results/tableB1", ///
...
ppml_panel_sg fa_outserv rta rta_age, ex(par) im(cou) y(year) 
outreg2 using "$home/output/results/tableB1", ///
append text addtext(Host-year FE, YES, Partner-year FE, YES, Country-pair FE, YES) label cttop("SERV")

ppml_panel_sg fa_outserv rta inv_prov rta_age, ex(par) im(cou) y(year) // 
outreg2 using "$home/output/results/tableB1", ///
